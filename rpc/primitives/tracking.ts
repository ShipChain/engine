...
import { Tracking } from '../../src/shipchain/vaults/primitives/Tracking';

@RPCNamespace({ name: 'Tracking' })
export class RPCTracking {
    @RPCMethod({
    ...
    public static async Get(args) {
        ...
        const tracking: Tracking = await vault.getPrimitive(PrimitiveType.Tracking.name);

        const content: any = await tracking.getTracking(wallet);
    ...
    public static async Add(args) {
        ...
        const document: Tracking = await vault.getPrimitive(PrimitiveType.Tracking.name);
        await document.addTracking(wallet, args.payload);
